...
class Net {
 public:
  ...
  // by cqiu
  Blob<Dtype>* getBlobByName(string name)
    {
  	  int id = blob_names_index_[name];
  	  return blobs_[id].get();
    }
  Layer<Dtype>* getLayerByName(string name)
    {
      int id = layer_names_index_[name];
      return layers_[id].get();
    }
