...
class Layer {
 ...
 protected:
  ...
  virtual void Forward_gpu(const vector<Blob<Dtype>*>& bottom,
    ...
    return Forward_cpu(0,bottom, top);
  }
...
// Log_Forward and backward wrappers. You should implement the cpu and
// gpu specific implementations instead, and should not change these
// functions.
template <typename Dtype>
inline Dtype Layer<Dtype>::Log_Forward(
    const int log_num_,
    const vector<Blob<Dtype>*>& bottom,
    const vector<Blob<Dtype>*>& top) {
  // Lock during forward to ensure sequential forward
  ...
  switch (Caffe::mode()) {
  case Caffe::CPU:
    Forward_cpu(log_num_,bottom, top);
    for (int top_id = 0; top_id < top.size(); ++top_id) {
      if (!this->loss(top_id)) { continue; }
      const int count = top[top_id]->count();
      const Dtype* data = top[top_id]->cpu_data();
    ...
    break;
  case Caffe::GPU:
    LOG(FATAL) << "UnSupport Log GPU caffe mode.";
    break;
  default:
    LOG(FATAL) << "Unknown caffe mode.";
  }
