...
////////Printf FeatureMap
 ...
 float maxWeight=-1024;
 float minWeight=1023;
...
//	  std::cout<<"featureBlob->width()="<<featureBlob->width()<<std::endl;
	  for (int j=0; j < featureBlob->height()*featureBlob->width(); j++){
		  for (int i = 0; i < featureBlob->channels(); i++){
			...
			float dataTmp = outPutFeature[i*featureBlob->height()*featureBlob->width()+j];
//			if(dataTmp > 255){
//				dataTmp = 256;//// just Test
//			}
//			dataTmp = 0;
			oFileData<<dataTmp<<std::endl;
		}
	 }
