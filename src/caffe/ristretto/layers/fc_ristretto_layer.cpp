...
template <typename Dtype>
void FcRistrettoLayer<Dtype>::log_file_fc_rto_layer_org(const vector<Blob<Dtype>*>& bottom){
    ////////==== debugPrint Begin by cqiu
    //long totalWeightNum =	this->blobs_[0]->channels()*this->blobs_[0]->num()*
    ...
    //long totalImageNum = bottom[0]->channels()*bottom[0]->height()*bottom[0]->width();
    static FILE *oFileStatic;
    char oFileNameStatic[256]={0};
    sprintf(oFileNameStatic,"cnnData/Weights/ORGFcWeightsFC.bin");
    oFileStatic = fopen(oFileNameStatic, "a+");
    int weightsTimes = 1 << this->fl_params_;
    for (int i = 0; i < bottom.size(); ++i) {
          printf("fc_bottom.size()        = %4d\n",int(bottom.size()));
          printf("fc_weights.channels     = %4d\n",int(this->blobs_[0]->channels()));
          printf("fc_weights.num          = %4d\n",int(this->blobs_[0]->num()));
          printf("fc_weights.height       = %4d\n",int(this->blobs_[0]->height()));
          printf("fc_weights.width        = %4d\n",int(this->blobs_[0]->width()));
          printf("fc_weightsBias.channels = %4d\n",int(this->blobs_[1]->channels()));
          printf("fc_weightsBias.num      = %4d\n",int(this->blobs_[1]->num()));
          printf("fc_weightsBias.height   = %4d\n",int(this->blobs_[1]->height()));
          printf("fc_weightsBias.width    = %4d\n",int(this->blobs_[1]->width()));
          printf("fc_img.channels         = %4d\n",int(bottom[0]->channels()));
          printf("fc_img.height           = %4d\n",int(bottom[0]->height()));
          printf("fc_img.width            = %4d\n",int(bottom[0]->width()));
          printf("fc_weightsTimes         = %4d\n",int(weightsTimes));
          //Load weights
          ...
          for(int co= 0 ; co < coTotal ; co++){
              for(int ci = 0 ; ci < ciTotal ;ci++){
                  for(int k = 0 ; k < kTotal ;k++){
                    int dataTmp = weightsTimes * (*(weightTmp+k+ci*kTotal+co*kTotal*ciTotal));
                    fprintf(oFileStatic,"%4d\n",dataTmp);
                  }
              }
          }
          //Load weightsBias
        int weightsTimesBias = 1 << this->fl_params_bias_;
        const Dtype * weightBiasTmp     = this->weights_quantized_[1]->cpu_data();
        for(int co= 0 ; co < coTotal ; co++){
            int dataTmp;
            dataTmp = weightsTimesBias * (*(weightBiasTmp+co));
            fprintf(oFileStatic,"%4d\n",dataTmp);
        }
        fclose(oFileStatic);
    }
...
template <typename Dtype>
void FcRistrettoLayer<Dtype>::Forward_cpu(const int log_num_,const vector<Blob<Dtype>*>& bottom,
    const vector<Blob<Dtype>*>& top) {
  // Trim layer input
  if (this->phase_ == TEST) {
...
#ifdef FC_RISTRETTO_LAYER_EN1
    log_file_fc_rto_layer_org(bottom);
#endif
