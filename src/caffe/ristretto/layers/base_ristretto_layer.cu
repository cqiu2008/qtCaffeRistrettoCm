...
void BaseRistrettoLayer<Dtype>::QuantizeWeights_gpu(
  ...
  case QuantizationParameter_Precision_DYNAMIC_FIXED_POINT:
    Trim2FixedPoint_gpu(weight, cnt_weight, bw_params_, rounding, fl_params_,fl_params_bias_);
    if (bias_term) {
      Trim2FixedPoint_gpu(weights_quantized[1]->mutable_gpu_data(),
          weights_quantized[1]->count(), bw_params_, rounding, fl_params_,fl_params_bias_);
    }
    break;
